...
async function initChain(label, endpoint, lbChainId, WinzaAppId) {
  log('init', label, 'endpoint=', endpoint)
  if (!endpoint) throw new Error('missing endpoint')
  if (!WinzaAppId) throw new Error('missing Winza app id')
  ...
  await runMutation(endpoint, label, 'setWinzaAppId', `mutation { setWinzaAppId(WinzaAppId: "${WinzaAppId}") }`)
...
async function start() {
  ...
  const WinzaId = process.env.WinzaREAL || process.env.VITE_Winza_APPLICATION_ID || process.env.VITE_LINERA_APPLICATION_ID
  ...
  await initChain('btc', BTC_HTTP, lbBtc, WinzaId)
  await initChain('eth', ETH_HTTP, lbEth, WinzaId)
}
