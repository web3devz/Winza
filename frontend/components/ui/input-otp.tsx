...
import { OTPInput as OriginalOTPInput } from "input-otp";
...
const OTPInputContext = React.createContext<OTPContextType | undefined>(undefined);

function InputOTP({
  className,
  containerClassName,
  ...props
}: React.ComponentProps<typeof OriginalOTPInput> & { containerClassName?: string }) {
  return (
    <OriginalOTPInput
      data-slot="input-otp"
...
function InputOTPSlot({ index, className, ...props }: React.ComponentProps<"div"> & { index: number }) {
  ...
  const { char, isActive, hasFakeCaret } = context?.slots[index] ?? {};
